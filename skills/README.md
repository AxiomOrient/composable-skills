...
| 설계안, 문서 초안, 구현 제안, PRD, 리서치를 3개 모델이 각각 만들고 합의로 좁히고 싶다 | [`workflow-consensus-engine`](./workflow-consensus-engine/SKILL.md) | 필요하면 compose 계약을 공유해서 세 모델이 각자 작업하고, 가장 강한 산출물과 남는 충돌을 분리한다 |
...
- [`workflow-consensus-engine`](./workflow-consensus-engine/SKILL.md): 범위와 완료 조건을 먼저 잠근 뒤, 필요하면 compose 매크로를 공유 실행 계약으로 써서 Codex, Claude Code, Gemini CLI가 각자 초안/제안/리포트를 만들고, 합의 엔진이 가장 강한 산출물과 남는 충돌을 정리한다.
...
- [`tidy-review`](./tidy-review/SKILL.md): 최근 변경분을 재사용, 구조 품질, 효율 세 렌즈로 한 번에 읽기 전용 리뷰한다.
