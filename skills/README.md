...
| 지금 필요한 것 | 먼저 고를 workflow | 왜 이걸 먼저 고르나 |
|---|---|---|
| 머릿속 주제가 흐리다 | [`workflow-ask-get-clear`](./workflow-ask-get-clear/SKILL.md) | 흐린 생각을 한 문장 문제로 묶어 준다 |
| 질문은 있는데 더 날카롭게 만들고 싶다 | [`workflow-ask-sharpen`](./workflow-ask-sharpen/SKILL.md) | 질문을 쪼개고 반대 질문까지 만들어 준다 |
| 현재 구조를 먼저 알고 싶다 | [`workflow-scout-structure`](./workflow-scout-structure/SKILL.md) | 범위를 정하고 지금 모양을 지도처럼 펼쳐 준다 |
| 리뷰를 체계적으로 받고 싶다 | [`workflow-check-full-review`](./workflow-check-full-review/SKILL.md) | 구조 파악과 리뷰 스캔을 함께 돌린다 |
| 리뷰와 체크리스트를 같이 보고 싶다 | [`workflow-check-with-checklist`](./workflow-check-with-checklist/SKILL.md) | 구조 리뷰에 품질 체크리스트까지 한 번에 더한다 |
...
| 기존 태스크 문서를 끝까지 자동 실행하고 싶다 | [`workflow-build-execute-loop`](./workflow-build-execute-loop/SKILL.md) | `plans/TASKS.md`를 읽고 루프를 계속 돈다 |
| 테스트가 어디 비었는지 찾고 메우고 싶다 | [`workflow-test-close-gaps`](./workflow-test-close-gaps/SKILL.md) | 빈 구멍 찾기부터 테스트 작성까지 이어 준다 |
| 코드나 구조를 더 단순하게 만들고 싶다 | [`workflow-tidy-find-improvements`](./workflow-tidy-find-improvements/SKILL.md) | 복잡성 지도부터 개선 포인트까지 정리한다 |
| 문서를 정리하고 새로 쓰고 싶다 | [`workflow-doc-systemize`](./workflow-doc-systemize/SKILL.md) | 문서 인벤토리, 정리, 작성까지 한 번에 이어 준다 |
| 커밋이나 릴리즈 전에 보안만 가볍게 점검하고 싶다 | [`workflow-security-preflight`](./workflow-security-preflight/SKILL.md) | 비밀키, ignore drift, 모바일 설정 노출 같은 위험을 가볍게 훑는다 |
| 배포 가능한지 먼저 판단하고 싶다 | [`workflow-ship-ready-check`](./workflow-ship-ready-check/SKILL.md) | 저장소 상태, 릴리즈 위생, 최종 출고 준비를 점검한다 |
| 릴리즈 점검부터 publish까지 한 번에 하고 싶다 | [`workflow-ship-it`](./workflow-ship-it/SKILL.md) | 점검, 커밋/태그, 공개까지 한 흐름으로 묶는다 |
...
- [`workflow-ask-get-clear`](./workflow-ask-get-clear/SKILL.md): 머릿속에 둥둥 떠다니는 주제를 한 문장 문제와 질문 묶음으로 바꿔 준다. "무슨 말을 해야 할지도 모르겠다"는 상태에 맞다.
- [`workflow-ask-sharpen`](./workflow-ask-sharpen/SKILL.md): 이미 있는 질문을 더 쪼개고, 무엇을 가정하고 있는지 흔들어 보면서 "진짜 물어야 할 질문"으로 벼린다.

### 범위와 현재 구조를 파악하는 workflow

- [`workflow-scout-structure`](./workflow-scout-structure/SKILL.md): 먼저 범위를 분명히 정한 다음, 그 안의 현재 구조를 지도로 그려 준다. 새 설계 전에 "지금 뭐가 어디에 있는지"부터 알고 싶을 때 맞다.

### 리뷰와 점검을 위한 workflow

- [`workflow-check-full-review`](./workflow-check-full-review/SKILL.md): 한 폴더나 모듈을 보고 구조 파악과 리뷰 스캔을 함께 수행한다. 막연한 "버그 좀 봐 줘"보다 훨씬 투명한 리뷰 흐름이다.
- [`workflow-check-with-checklist`](./workflow-check-with-checklist/SKILL.md): 위 리뷰 흐름에 9개 품질 체크리스트까지 더한 버전이다. 구조 리뷰와 정해진 품질표를 둘 다 원할 때 쓴다.
...
- [`workflow-build-execute-loop`](./workflow-build-execute-loop/SKILL.md): 이미 `plans/TASKS.md` 같은 태스크 장부가 있을 때, 그 장부를 따라 구현하고 점검하고 다시 sync하는 루프를 끝까지 돌린다.
...
- [`workflow-tidy-find-improvements`](./workflow-tidy-find-improvements/SKILL.md): 복잡한 곳, 중복된 곳, 정리하면 좋아질 곳을 먼저 지도처럼 보여 준다. 막연한 "좀 더 깔끔하게"를 구체적인 개선 후보로 바꾼다.

### 문서를 정리하는 workflow

- [`workflow-doc-systemize`](./workflow-doc-systemize/SKILL.md): 문서를 전수 조사하고, 겹치거나 오래된 문서를 정리하고, 필요한 문서를 다시 쓴다. 문서판 대청소와 재배치를 한 번에 하는 흐름이다.

### 보안과 릴리즈를 다루는 workflow

- [`workflow-security-preflight`](./workflow-security-preflight/SKILL.md): 커밋이나 릴리즈 직전에 비밀 정보 노출, ignore 누락, 모바일 설정 노출 같은 문제를 가볍고 빠르게 걸러낸다.
- [`workflow-ship-ready-check`](./workflow-ship-ready-check/SKILL.md): 저장소가 실제로 릴리즈할 상태인지, 문서와 위생이 맞는지, 최종 출고해도 되는지 차분하게 점검한다.
- [`workflow-ship-it`](./workflow-ship-it/SKILL.md): 릴리즈 준비 점검에서 끝나지 않고, release-only 커밋과 태그, publish 흐름까지 묶어 실제 출고까지 밀어 준다.
...
- [`ask-form-question`](./ask-form-question/SKILL.md): 흐릿한 주제를 한 문장 질문으로 묶는다. 아무 말이나 많을 때 핵심 한 줄을 잡는 공구다.
...
### `scout-` — 현재 상태를 조사하는 공구

이 묶음은 "지금 여기서 무슨 일이 벌어지고 있는가"를 파악하는 공구다.
바로 고치기 전에 지형도를 그린다.

- [`scout-scope`](./scout-scope/SKILL.md): 모호한 요청을 범위, 제약, 완료 기준으로 정리한다. 아직 일의 테두리가 흐릴 때 쓴다.
- [`scout-boundaries`](./scout-boundaries/SKILL.md): 어디까지가 이번 일이고 어디부터가 아닌지 경계를 선명하게 그린다.
- [`scout-structure-map`](./scout-structure-map/SKILL.md): 현재 구조를 지도처럼 그린다. 파일, 책임, 경계가 어떻게 나뉘는지 보고 싶을 때 맞다.
- [`scout-option-compare`](./scout-option-compare/SKILL.md): 몇 가지 선택지를 나란히 놓고 장단점을 비교한다. "A로 갈까 B로 갈까"를 따질 때 쓴다.
- [`scout-evidence-gap`](./scout-evidence-gap/SKILL.md): 결론을 믿기 전에 빠진 근거가 무엇인지 찾는다. 자신감이 아니라 증거가 필요한 순간에 맞다.
- [`scout-baseline`](./scout-baseline/SKILL.md): 성능이나 수치를 개선하기 전에 현재 기준선을 정확히 찍어 둔다. 빠르게 만들기 전에 먼저 재는 공구다.
...
이 묶음은 "좋아 보인다"가 아니라 "어디가 위험한지"를 드러내는 공구다.
특히 코드 리뷰, 경계 점검, 최종 검증에 맞다.

- [`check-merge-ready`](./check-merge-ready/SKILL.md): PR 리뷰처럼 문제를 우선순위와 근거와 함께 정리한다. 승인할지 보류할지 판단하는 공구다.
- [`check-quality-scan`](./check-quality-scan/SKILL.md): 정해진 9개 품질 항목으로 바뀐 코드를 훑는다. 체크리스트 검사관에 가깝다.
- [`check-failure-paths`](./check-failure-paths/SKILL.md): 정상 경로가 아니라 실패 경로만 집중해서 본다. 예외 처리와 복구가 약한지 점검할 때 맞다.
- [`check-module-bounds`](./check-module-bounds/SKILL.md): 모듈이나 API 경계가 흐린지, 책임이 새는지 확인한다.
- [`check-security-holes`](./check-security-holes/SKILL.md): 보안 노출, 비밀 정보 유출, 취약 지점을 우선순위와 함께 검토한다.
- [`check-ship-risk`](./check-ship-risk/SKILL.md): 지금 이 변경을 합치거나 배포할 때 어떤 위험이 남는지 본다. 코드 수정이 아니라 위험 판정용이다.
- [`check-final-verify`](./check-final-verify/SKILL.md): 작업이 끝난 뒤, 약속한 계약이 정말 충족됐는지 마지막으로 확인한다.
- [`check-improve-loop`](./check-improve-loop/SKILL.md): 결과물을 보고 가장 값비싼 흠을 하나씩 고치며 품질 문턱까지 반복한다. 자기 피드백 루프 공구다.
...
- [`tidy-why-complex`](./tidy-why-complex/SKILL.md): 지금 왜 복잡한지 원인을 먼저 분해한다. 본질적인 복잡함과 쓸데없는 복잡함을 구분하는 공구다.
...
- [`tidy-simplify`](./tidy-simplify/SKILL.md): 실제 코드에서 복잡한 구조를 더 짧고 이해하기 쉬운 모양으로 단순화한다.
- [`tidy-remove-legacy`](./tidy-remove-legacy/SKILL.md): 오래된 파일, 다 끝난 계획 문서, 죽은 별칭, 쓸모없는 접착 코드 같은 잔재를 치운다.

### `doc-` — 문서를 다루는 공구

이 묶음은 코드가 아니라 설명을 정리하는 공구다.
무엇이 있는지 찾고, 정리하고, 쓰고, 입구 문서를 만든다.

- [`doc-find-all`](./doc-find-all/SKILL.md): 문서를 전수 조사해서 고아 문서, 중복 문서, 낡은 문서를 찾는다.
- [`doc-curate`](./doc-curate/SKILL.md): 루트 README 아래쪽 문서들을 정리하고, 연결하고, 중복을 걷어내고, 진입 문서를 다시 세운다.
- [`doc-write`](./doc-write/SKILL.md): 아키텍처 노트, 사용 가이드, 개념 설명 같은 비루트 문서를 실제로 쓴다.
- [`doc-build-index`](./doc-build-index/SKILL.md): 모듈 묶음, 라이브러리, 논문처럼 계층이 있는 대상을 읽기 좋은 인덱스 문서 세트로 엮는다.
...
- [`ship-check-repo`](./ship-check-repo/SKILL.md): 이 저장소가 실제로 릴리즈 가능한 git 상태인지, 브랜치와 remote가 맞는지부터 확인한다.
- [`ship-check-hygiene`](./ship-check-hygiene/SKILL.md): 낡은 파일이 남아 있지 않은지, 공개 표면과 문서가 어긋나지 않는지 릴리즈 위생을 본다.
- [`ship-release-verdict`](./ship-release-verdict/SKILL.md): 배포 범위, 롤아웃, 롤백 준비를 보고 GO/NO-GO 판단을 내린다.
- [`ship-commit`](./ship-commit/SKILL.md): 이미 끝난 변경을 바탕으로 Conventional Commit 메시지 후보를 만든다.
...
- [`build-until-done`](./build-until-done/SKILL.md): 코드 변경 작업을 "끝났다는 증거"가 나올 때까지 반복해서 밀어 주는 제어 엔진이다.
- [`finish-until-done`](./finish-until-done/SKILL.md): 문서, 리뷰, 계획처럼 비코드 작업을 끝날 때까지 반복해서 밀어 주는 제어 엔진이다.
...
- [`respond`](./respond/SKILL.md): 마지막 결과를 사람에게 읽기 쉬운 짧고 명확한 말로 바꾸는 출력 전용 공구다.
...
- 끝날 때까지 반복 실행이 필요하다: `build-until-done` 또는 `finish-until-done`을 본다.
