...
| 리뷰를 체계적으로 받고 싶다 | [`workflow-review-change`](./workflow-review-change/SKILL.md) | 구조 파악과 리뷰 스캔을 함께 돌린다 |
...
| 보안 노출만 좁게 보고 싶다 | [`check-security-holes`](./check-security-holes/SKILL.md) | 비밀키, ignore drift, 공개 설정 노출 같은 위험만 좁게 본다 |
...
- 체크리스트 관점이 추가로 필요하면 `workflow-review-change` 뒤에 `check-quality-scan`을 compose하는 쪽이 기본이다.
...
- [`check-security-holes`](./check-security-holes/SKILL.md): 비밀 정보 노출, ignore drift, 공개 설정 누락 같은 위험을 점검한다.
...
- [`scout-scope-contract`](./scout-scope-contract/SKILL.md): 모호한 요청을 범위, 제약, 완료 기준으로 정리한다. 아직 일의 테두리가 흐릴 때 쓴다.
...
- [`check-change-review`](./check-change-review/SKILL.md): PR 리뷰처럼 문제를 우선순위와 근거와 함께 정리한다. 승인할지 보류할지 판단하는 공구다.
