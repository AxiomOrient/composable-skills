...
def run_one_agent(
    ...
    for attempt in (1, 2):
        ...
        if agent == "codex":
            ...
            command = codex_command(resolved_cmd, strict_prompt, schema_path, response_path)
        elif agent == "claude":
            response_path = stdout_path
            command = claude_command(resolved_cmd, strict_prompt, schema_text)
        else:
            response_path = stdout_path
            command = gemini_command(resolved_cmd, strict_prompt)
